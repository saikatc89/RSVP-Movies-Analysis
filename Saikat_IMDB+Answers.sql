...
select SUM(CASE when m.id is null then 1 else 0 end ) as id_total_null_values,
		SUM(CASE when m.title is null then 1 else 0 end ) as title_total_null_values,
        SUM(CASE when m.year is null then 1 else 0 end ) as year_total_null_values,
        SUM(CASE when m.date_published is null then 1 else 0 end ) as date_total_null_values,
        SUM(CASE when m.duration is null then 1 else 0 end ) as duration_total_null_values,
        SUM(CASE when m.country is null then 1 else 0 end ) as country_total_null_values,
        SUM(CASE when m.worlwide_gross_income is null then 1 else 0 end ) as world_total_null_values,
        SUM(CASE when m.languages is null then 1 else 0 end ) as language_total_null_values,
        SUM(CASE when m.production_company is null then 1 else 0 end ) as production_total_null_values
        from movie as m;
...
select country, year, count(*) as Total_number_of_movies_released
from movie
...
select distinct g.Genre,year,COUNT(m.title) AS total_movies_produced
from Genre AS g
...
select production_company,count(*) as movie_count,dense_rank() over(order by count(id) desc) as prod_company_rank
...
select country, sum(total_votes) as toatl_votes
from movie as m 
join ratings as r
on r.movie_id=m.id
